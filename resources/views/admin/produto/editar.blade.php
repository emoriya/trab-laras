    ...
    <div class="card-body">
        <form method="post" enctype="multipart/form-data" action="{{route('produtos.update', $produto->id)}}">
            ...
            <div class="form-group">
                ...
                <input name="nome" type="text" class="form-control" id="inputNome" placeholder="Nome do Produto" value="{{$produto->nome}}">
            </div>

            <div class="form-group">
                <label for="inputPreco">Preço (R$)</label>
                <input name="preco" type="text" class="form-control" id="inputPreco" placeholder="Preço" value="{{$produto->preco}}">
            </div>

            <div class="form-group">
                <label for="inputDescricaoBreve">Descriçao Breve</label>
                <textarea name="descricao_breve" class="form-control" id="inputDescricaoBreve" cols="30" rows="10">{{$produto->descricao_breve}}</textarea>
            </div>

            <div class="form-group">
                <label for="inputDescricaoCompleta">Descriçao Completa</label>
                <textarea name="descricao_completa" class="form-control" id="inputDescricaoCompleta" cols="30" rows="10">{{$produto->descricao_completa}}</textarea>
            </div>

            <div class="form-group">
                <label for="inputSituacao">Situaçao</label>
                <select class="form-control" name="situacao" id="inputSituacao">
                    <option value="1" {{ $produto->situacao == 1 ? "selected" : "" }} > Ativo </option>
                    <option value="0" {{ $produto->situacao == 0 ? "selected" : "" }}> Inativo </option>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="servico_id" id="inputServicoId">
                    @foreach($servicosList as $servico)
                        <option {{ $produto->servico_id == $servico->id ? "selected" : "" }} value="{{$servico->id}}"> {{$servico->titulo}} </option>
                    @endforeach
                </select>
            </div>
